#!/usr/bin/env bash
# ============================================================================
#  Al Islam wal Manahij Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/12-al-islam-wal-manahij}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alislamwalmanahij}"
AVD_NAME="${AVD_NAME:-al_islam_wal_manahij_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-islam-wal-manahij-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Islam wal Manahij Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}

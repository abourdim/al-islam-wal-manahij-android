...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','cards','comparisons'],
    tabIndex: 0,
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'الإسلام والمناهج الاشتراكية' : l === 'fr' ? 'L\'Islam et les Systemes Socialistes' : 'Islam and Socialist Systems',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('manahij-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('manahij-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('manahij-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('manahij-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('manahij-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('manahij-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('manahij-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('manahij-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('manahij-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('manahij-narrator-duo') === 'true';
  }
  ...
  if (origRenderCards) {
    window.renderCards = function() { origRenderCards(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderComparisons = window.renderComparisons;
  if (origRenderComparisons) {
    window.renderComparisons = function() { origRenderComparisons(); setTimeout(injectSpeakButtons, 100); };
  }
